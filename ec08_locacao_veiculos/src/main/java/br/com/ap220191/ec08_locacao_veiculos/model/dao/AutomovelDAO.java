...
public class AutomovelDAO {
    ...
    public boolean update(Automovel automovel) throws SQLException {
        try{     
        String sql = "UPDATE veiculos SET placa = false WHERE tipo = '"+automovel.getTipo()+"')";
        PreparedStatement statement = connection.prepareStatement(sql);
    ...
    public ResultSet existeAutomovel(Automovel automovel) throws SQLException {
        
        String sql = "select * from veiculos where tipo = '"+automovel.getTipo()+"'";
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.execute();      
